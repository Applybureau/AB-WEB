...
router.post('/:id/request-new-availability', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      await sendEmail(consultation.email, 'request_new_consultation_times', {
        client_name: consultation.full_name,
...
router.post('/:id/complete', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const updateData = {
      status: 'completed',
      workflow_stage: outcome === 'proceeding' ? 'awaiting_payment' : 'consultation_completed_no_proceed',
    ...
    try {
      if (outcome === 'proceeding') {
        await sendEmail(consultation.email, 'consultation_proceeding', {
          client_name: consultation.full_name,
        ...
        await sendEmail(consultation.email, 'consultation_completed_no_proceed', {
          client_name: consultation.full_name,

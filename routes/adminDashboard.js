...
router.get('/overview', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    try {
      const { count: consultationsCount } = await supabaseAdmin
        .from('consultation_requests')
        .select('*', { count: 'exact', head: true });
      stats.total_consultations = consultationsCount || 0;
    } catch (error) {
    ...
    try {
      const { count: applicationsCount } = await supabaseAdmin
        .from('applications')
        .select('*', { count: 'exact', head: true });
      stats.total_applications = applicationsCount || 0;
    } catch (error) {
    ...
    try {
      ...
      stats.pending_onboarding = pendingCount || 0;
    } catch (error) {
      console.error('Error counting pending onboarding:', error);
    }
    ...
    try {
      ...
      stats.active_consultations = activeCount || 0;
    } catch (error) {
      console.error('Error counting active consultations:', error);
    }
    ...
    try {
      ...
      stats.recent_applications = recentCount || 0;
    } catch (error) {
      console.error('Error counting recent applications:', error);
    }

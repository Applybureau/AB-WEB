...
router.get('/', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Build base query for clients with their 20Q data
    ...
    if (error) {
      console.error('Error fetching clients with 20Q status:', error);
      return res.status(500).json({ error: 'Failed to fetch clients' });
    }

    // Format the response with enhanced 20Q status information
    const clientsWithStatus = clients.map(client => {
      const onboarding20q = client.client_onboarding_20q?.[0] || null;
      ...
      return {
        ...
        twenty_questions: format20QuestionsStatusForAdmin(onboarding20q),
    ...
    // Apply status filter after formatting if provided
    let filteredClients = clientsWithStatus;
    if (status_filter && status_filter !== 'all') {
      filteredClients = clientsWithStatus.filter(client => 
        client.twenty_questions.status === status_filter
      );
    }

    // Get status summary for dashboard metrics
    const statusSummary = {
      not_started: clientsWithStatus.filter(c => c.twenty_questions.status === 'not_started').length,
      pending_approval: clientsWithStatus.filter(c => c.twenty_questions.status === 'pending_approval').length,
      active: clientsWithStatus.filter(c => c.twenty_questions.status === 'active').length,
      paused: clientsWithStatus.filter(c => c.twenty_questions.status === 'paused').length,
      completed: clientsWithStatus.filter(c => c.twenty_questions.status === 'completed').length,
    ...
    res.status(500).json({ error: 'Failed to load 20Q dashboard' });
  }
});
...
router.get('/client/:clientId', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Get client with detailed 20Q information

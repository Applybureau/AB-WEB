-- Client Dashboard Schema Migration (Fixed for Existing Tables)
...
-- ============================================
-- 2. STRATEGY CALLS (Already exists, skip)
-- ============================================
-- strategy_calls table already exists with correct structure
...
-- ============================================
-- Note: clients table already has:
--   • onboarding_complete (we'll add onboarding_completed as alias)
--   • profile_unlocked (already exists!)
--   • payment_verified (we'll add payment_confirmed as alias)
...
-- Add payment_confirmed column if it doesn't exist (alias for payment_verified)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Note: profile_unlocked already exists in clients table, no need to add

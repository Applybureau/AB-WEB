...
class AdminController {
  ...
  // GET /api/admin/clients/:clientId/20q/responses - Get client's 20Q responses
  static async get20QResponses(req, res) {
    try {
      ...
      // Get 20Q responses
      const { data: twentyQ, error } = await supabaseAdmin
        .from('twenty_questions')
        .select('*')
        .eq('client_id', clientId)
        .single();

      if (error && error.code !== 'PGRST116') {
        logger.error('Error fetching 20Q responses', error, { clientId });
        return res.status(500).json({ error: 'Failed to fetch 20Q responses' });
      }

      if (!twentyQ) {
        return res.status(404).json({ error: '20Q responses not found for this client' });
      }
      ...
      res.json({
        ...
        twenty_questions: {
          id: twentyQ.id,
          status: twentyQ.status || 'under_review',
          submitted_at: twentyQ.submitted_at,
          reviewed_at: twentyQ.reviewed_at,
          reviewed_by: twentyQ.reviewed_by,
          admin_notes: twentyQ.admin_notes,
          approved: twentyQ.approved,
          responses: twentyQ.responses || {}
        }
      });
    } catch (error) {
      logger.error('Get 20Q responses error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to fetch 20Q responses' });
    }
  }

  // POST /api/admin/clients/:clientId/20q/mark-reviewed - Mark 20Q as reviewed
  static async mark20QReviewed(req, res) {
    try {
      const { clientId } = req.params;
      ...
      logger.info('Admin marking 20Q as reviewed', { clientId, adminId, approved });

      // Get current 20Q status
      const { data: currentTwentyQ, error: fetchError } = await supabaseAdmin
        .from('twenty_questions')
        .select('status')
        .eq('client_id', clientId)
        .single();

      if (fetchError || !currentTwentyQ) {
        return res.status(404).json({ error: '20Q responses not found for this client' });
      }

      const previousStatus = currentTwentyQ.status;
      ...
      if (error) {
        logger.error('Error marking 20Q as reviewed', error, { clientId, adminId });
        return res.status(500).json({ error: 'Failed to mark 20Q as reviewed' });
      }

      // Update client dashboard status
      const { data: client, error: clientError } = await supabaseAdmin
        .from('clients')
      ...
      if (approved && client) {
        try {
          await sendEmail(client.email, 'message_notification', {
            client_name: client.full_name,
            message_subject: '20 Questions Review Complete',
            message_content: feedback || 'Your 20 Questions responses have been reviewed and approved. You can now proceed with your application process.',
            dashboard_link: `${process.env.FRONTEND_URL}/dashboard`
          });
        } catch (emailError) {
          logger.error('Failed to send 20Q review email', emailError);
        }
      }

      logger.info('20Q marked as reviewed successfully', {
        clientId,
        adminId,
        previousStatus,
        approved
      ...
      res.json({
        ...
        new_dashboard_status: {
          twenty_questions: {
            ...
            description: 'Your responses have been reviewed by our team',
            color: 'green',
            progress: 100,
            can_edit: false
          }
      ...
      logger.error('Mark 20Q reviewed error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to mark 20Q as reviewed' });
    }
  }

  // GET /api/admin/20q/pending-review - Get all pending 20Q reviews
  static async getPending20QReviews(req, res) {
    try {
      const { page = 1, limit = 20, sort = 'submitted_at', order = 'desc' } = req.query;
      const offset = (parseInt(page) - 1) * parseInt(limit);

      // Get pending 20Q submissions
      let query = supabaseAdmin
        .from('twenty_questions')
        .select(`
          id,
        ...
        `)
        .in('status', ['under_review', 'pending_approval', 'submitted'])
        .order(sort, { ascending: order === 'asc' })
        .range(offset, offset + parseInt(limit) - 1);

      const { data: twentyQs, error } = await query;

      if (error) {
        logger.error('Error fetching pending 20Q reviews', error);
        return res.status(500).json({ error: 'Failed to fetch pending reviews' });
      }

      // Calculate days pending and priority
      const pending_reviews = (twentyQs || []).map(tq => {
        const submittedDate = new Date(tq.submitted_at);
        ...
        return {
          ...
          client_name: tq.clients.full_name,
          client_email: tq.clients.email,
          status: tq.status,
          submitted_at: tq.submitted_at,
          days_pending: daysPending,
          response_count: Object.keys(tq.responses || {}).length,
          priority
        };
      });
      ...
      const summary = {
        ...
        urgent: pending_reviews.filter(r => r.priority === 'urgent').length,
        high_priority: pending_reviews.filter(r => r.priority === 'high').length,
        normal: pending_reviews.filter(r => r.priority === 'normal').length
      };
      ...
      logger.error('Get pending 20Q reviews error', error);
      res.status(500).json({ error: 'Failed to fetch pending reviews' });
    }
  }

  // ==================== CLIENT FILES ENDPOINTS ====================

  // GET /api/admin/clients/:clientId/files - Get all client files
  static async getClientFiles(req, res) {
    try {
      ...
      const { data: client, error: clientError } = await supabaseAdmin
        .from('clients')
        .select('id, full_name, email, resume_url, linkedin_url, portfolio_url')
        .eq('id', clientId)
        .single();
      ...
      const resume = client.resume_url ? {
        id: 'resume',
        filename: client.resume_url.split('/').pop(),
        file_type: 'resume',
        file_url: client.resume_url,
        uploaded_at: null,
        version: 1,
        status: 'active'
      } : null;
      ...
      const linkedin = client.linkedin_url ? {
        id: 'linkedin',
        url: client.linkedin_url,
        file_type: 'linkedin',
        added_at: null,
        verified: true
      } : null;

      const portfolio = client.portfolio_url ? [{
        id: 'portfolio',
        url: client.portfolio_url,
        file_type: 'portfolio',
        title: 'Portfolio',
        added_at: null,
        verified: true
      ...
      res.json({
        ...
        summary: {
          ...
          linkedin_added: !!linkedin,
          portfolio_added: portfolio.length > 0,
          total_files: (files || []).length
        },
        resume,
        linkedin,
        portfolio,
        other_files: (files || []).map(file => ({
          id: file.id,
          filename: file.name,
          file_type: 'document',
          file_url: `${process.env.SUPABASE_URL}/storage/v1/object/public/client-files/${clientId}/${file.name}`,
          uploaded_at: file.created_at
        }))
      });
    } catch (error) {
      logger.error('Get client files error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to fetch client files' });
    }
  }

  // GET /api/admin/clients/:clientId/files/resume - Get resume details
  static async getClientResume(req, res) {
    try {
      const { clientId } = req.params;
      ...
      if (!client.resume_url) {
        return res.status(404).json({ error: 'Resume not found for this client' });
      }

      const filename = client.resume_url.split('/').pop();
      ...
      res.json({
        resume: {
          ...
          filename,
          file_url: client.resume_url,
          download_url: client.resume_url,
      ...
      logger.error('Get client resume error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to fetch resume' });
    }
  }

  // GET /api/admin/clients/:clientId/files/linkedin - Get LinkedIn details
  static async getClientLinkedIn(req, res) {
    try {
      const { clientId } = req.params;
      ...
      if (!client.linkedin_url) {
        return res.status(404).json({ error: 'LinkedIn profile not found for this client' });
      }
      ...
      logger.error('Get client LinkedIn error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to fetch LinkedIn profile' });
    }
  }

  // GET /api/admin/clients/:clientId/files/portfolio - Get portfolio details
  static async getClientPortfolio(req, res) {
    try {
      const { clientId } = req.params;
      ...
      logger.error('Get client portfolio error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to fetch portfolio' });
    }
  }

  // ==================== PACKAGE MONITOR ENDPOINTS ====================

  // GET /api/admin/clients/:clientId/package - Get client package details
  static async getClientPackage(req, res) {
    try {
      const { clientId } = req.params;

      const { data: client, error } = await supabaseAdmin
        .from('clients')
        .select('id, package_tier, package_start_date, package_expiry_date, package_status, auto_renewal')
        .eq('id', clientId)
        .single();
      ...
      const expiryDate = client.package_expiry_date ? new Date(client.package_expiry_date) : null;
      const daysRemaining = expiryDate ? Math.ceil((expiryDate - new Date()) / (1000 * 60 * 60 * 24)) : null;

      // Determine status
      let status = client.package_status || 'active';
      ...
      res.json({
        ...
        package_name: client.package_tier || 'Basic',
        start_date: client.package_start_date,
        expiry_date: client.package_expiry_date,
        days_remaining: daysRemaining,
        status,
        auto_renewal: client.auto_renewal || false,
        payment_status: 'paid'
      });
    } catch (error) {
      logger.error('Get client package error', error, { clientId: req.params.clientId });
      res.status(500).json({ error: 'Failed to fetch package details' });
    }
  }

  // GET /api/admin/packages/expiring - Get expiring packages
  static async getExpiringPackages(req, res) {
    try {
      ...
      const { data: clients, error } = await supabaseAdmin
        .from('clients')
        .select('id, full_name, email, package_tier, package_expiry_date, last_login_at')
        .not('package_expiry_date', 'is', null)
        .lte('package_expiry_date', thresholdDate.toISOString())
        .gte('package_expiry_date', new Date().toISOString())
        .order('package_expiry_date', { ascending: true })
        .range(offset, offset + parseInt(limit) - 1);
      ...
      const expiring_packages = (clients || []).map(client => {
        const expiryDate = new Date(client.package_expiry_date);
        ...
        return {
          ...
          last_activity: client.last_login_at
        };
      });
      ...
      res.status(500).json({ error: 'Failed to fetch expiring packages' });
    }
  }

  // PUT /api/admin/clients/:clientId/package/extend - Extend client package
  static async extendClientPackage(req, res) {
    try {
      const { clientId } = req.params;
      ...
      const { data: client, error: fetchError } = await supabaseAdmin
        .from('clients')
        .select('id, full_name, email, package_expiry_date')
        .eq('id', clientId)
        .single();
      ...
      const currentExpiry = client.package_expiry_date ? new Date(client.package_expiry_date) : new Date();
      const newExpiry = new Date(currentExpiry);
      newExpiry.setDate(newExpiry.getDate() + parseInt(extension_days));
      ...
      if (updateError) {
        logger.error('Error extending package', updateError, { clientId, adminId });
        return res.status(500).json({ error: 'Failed to extend package' });
      }

      logger.info('Package extended', {
        clientId,
        adminId,
      ...
      logger.error('Extend package error', error, { clientId: req.params.clientId });

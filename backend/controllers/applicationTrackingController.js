...
class ApplicationTrackingController {
  ...
  static async getApplicationStats(req, res) {
    try {
      ...
      // If admin, get overall stats; if client, get their specific stats
      if (userRole === 'admin') {
        const stats = await this.calculateOverallStats();
        res.json(stats);
      } else {
        const stats = await this.calculateApplicationStats(userId);
        res.json(stats);
      }
    } catch (error) {
  ...
  // Helper method to calculate overall statistics for admin
  static async calculateOverallStats() {
    try {
      ...
      if (error) {
        logger.error('Calculate overall stats error', error);
      ...
      return {
        ...
        overall_response_rate: allApplications.length > 0 
          ? Math.round(((statusCounts.interviewing || 0) + (statusCounts.offer || 0)) / allApplications.length * 100)
          : 0,
        overall_offer_rate: allApplications.length > 0

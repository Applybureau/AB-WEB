  ...
  try {
    ...
    if (unreadStats) {
      unreadStats.forEach(notification => {
        ...
        // Extract category and priority from metadata if available
        if (notification.metadata) {
          const category = notification.metadata.category;
          const priority = notification.metadata.priority;
          
          if (category) {
            stats.by_metadata[`category_${category}`] = (stats.by_metadata[`category_${category}`] || 0) + 1;
          }
          if (priority) {
            stats.by_metadata[`priority_${priority}`] = (stats.by_metadata[`priority_${priority}`] || 0) + 1;
          }
        }
      });
    }
...
router.post('/test', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Create notification directly using the correct schema
    const { data: notification, error } = await supabaseAdmin
      .from('notifications')
      .insert({
        user_id: userId,
        user_type: 'client', // Assuming it's for a client
        type: type,
        title: title,
        message: message,
        is_read: false,
...
router.get('/admin/stats', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { data: allNotifications, error } = await supabaseAdmin
      .from('notifications')
      .select('type, is_read, created_at, metadata, user_type');
    ...
    if (allNotifications) {
      allNotifications.forEach(notification => {
        ...
        // User type stats
        stats.by_user_type[notification.user_type] = (stats.by_user_type[notification.user_type] || 0) + 1;

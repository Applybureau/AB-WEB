...
router.patch('/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (communication_method !== undefined) updateData.communication_method = communication_method;
    if (whatsapp_number !== undefined) updateData.whatsapp_number = whatsapp_number;
    if (admin_whatsapp_number !== undefined) updateData.admin_whatsapp_number = admin_whatsapp_number;
    ...
    try {
      // Prepare WhatsApp information if communication method is WhatsApp
      let whatsappInfo = {};
      if (updatedConsultation.communication_method === 'whatsapp_call') {
        whatsappInfo = whatsappManager.createWhatsAppInstructions(updatedConsultation);
      }
      ...
      switch (status) {
        case 'confirmed':
          await sendEmail(clientEmail, 'consultation_confirmed', {
            ...
            communication_method: updatedConsultation.communication_method,
            whatsapp_number: whatsappInfo.adminNumber,
            whatsapp_instructions: whatsappInfo.instructions,
            whatsapp_web_link: whatsappInfo.whatsappWebLink,
            is_whatsapp_call: updatedConsultation.communication_method === 'whatsapp_call',
            is_video_call: updatedConsultation.communication_method === 'video_call',
...
// POST /api/consultation-management/:id/confirm-with-method - Confirm consultation with communication method
router.post('/:id/confirm-with-method', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { 
      selected_slot_index,
    ...
    if (!validMethods.includes(communication_method)) {
      return res.status(400).json({ 
        error: `Invalid communication method. Valid options: ${validMethods.join(', ')}` 
      });
    }
    ...
    if (selected_slot_index !== undefined) {
      const slots = consultation.preferred_slots;
      if (!Array.isArray(slots) || !slots[selected_slot_index]) {
    ...
    const updateData = {
      status: 'confirmed',
      communication_method,
      admin_notes: admin_notes || consultation.admin_notes,
      updated_at: new Date().toISOString()
    };

    // Handle slot selection
    if (selected_slot_index !== undefined && consultation.preferred_slots) {
      const selectedSlot = consultation.preferred_slots[selected_slot_index];
      updateData.scheduled_at = `${selectedSlot.date}T${selectedSlot.time}:00Z`;
    }

    // Add communication-specific fields
    if (communication_method === 'video_call' && meeting_link) {
      updateData.meeting_link = meeting_link;
    } else if (communication_method === 'whatsapp_call') {
      if (whatsapp_number) {
        // Validate WhatsApp number
        if (!whatsappManager.isValidWhatsAppNumber(whatsapp_number)) {
          return res.status(400).json({ error: 'Invalid WhatsApp number format' });
        }
        updateData.whatsapp_number = whatsappManager.formatWhatsAppNumber(whatsapp_number);
      }
      if (admin_whatsapp_number) {
        updateData.admin_whatsapp_number = whatsappManager.formatWhatsAppNumber(admin_whatsapp_number);
      }
    }
    ...
    let emailData = {
      ...
      consultation_time: new Date(updatedConsultation.scheduled_at).toLocaleTimeString(),
      communication_method: updatedConsultation.communication_method,
      is_whatsapp_call: communication_method === 'whatsapp_call',
      is_video_call: communication_method === 'video_call',
      meeting_details: meeting_details || 'Your consultation has been confirmed.',
      current_year: new Date().getFullYear()
    };

    // Add communication-specific email data
    if (communication_method === 'whatsapp_call') {
      const whatsappInfo = whatsappManager.createWhatsAppInstructions(updatedConsultation);
      emailData = {
        ...emailData,
        whatsapp_number: whatsappInfo.adminNumber,
        whatsapp_instructions: whatsappInfo.instructions,
      ...
      // Log WhatsApp consultation confirmation
      whatsappManager.logWhatsAppActivity(id, 'consultation_confirmed', {
        clientName,
        clientEmail,
        adminWhatsApp: whatsappInfo.adminNumber,
        scheduledAt: updatedConsultation.scheduled_at
      });
    } else if (communication_method === 'video_call') {
      emailData.meeting_link = updatedConsultation.meeting_link;
    }

    // Send confirmation email
    try {
      await sendEmail(clientEmail, 'consultation_confirmed', emailData);
    } catch (emailError) {
      console.error('Failed to send confirmation email:', emailError);
    ...
    try {
      await NotificationHelpers.createNotification({
        ...
        message: `Consultation confirmed for ${clientName} via ${communication_method.replace('_', ' ')}`,
        data: {
          consultation_id: id,
          communication_method,
          scheduled_at: updatedConsultation.scheduled_at
    ...
    res.json({
      ...
      consultation: {
        ...
        scheduled_at: updatedConsultation.scheduled_at,
        meeting_link: updatedConsultation.meeting_link,
        whatsapp_number: updatedConsultation.admin_whatsapp_number,
        client_whatsapp: updatedConsultation.whatsapp_number
      },
      confirmed_slot: selected_slot_index !== undefined ? consultation.preferred_slots[selected_slot_index] : null,
      confirmed_time: updatedConsultation.scheduled_at
    });
  } catch (error) {
    console.error('Confirm consultation with method error:', error);

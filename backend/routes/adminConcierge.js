...
router.post('/consultations/:id/confirm', async (req, res) => {
  try {
    ...
    try {
      // Extract meeting details properly if it's an object
      let meetingDetailsText = 'Your consultation has been confirmed.';
      let meetingLinkUrl = meeting_link || null;
      
      if (meeting_details) {
        if (typeof meeting_details === 'object') {
          // If meeting_details is an object, extract the notes
          meetingDetailsText = meeting_details.meeting_notes || meetingDetailsText;
          // Use meeting_link from object if not provided separately
          if (!meetingLinkUrl && meeting_details.meeting_link) {
            meetingLinkUrl = meeting_details.meeting_link;
          }
        } else if (typeof meeting_details === 'string') {
          meetingDetailsText = meeting_details;
        }
      }
...
router.post('/onboarding/:id/approve', async (req, res) => {
  try {
    ...
    try {
      await sendEmail(onboarding.registered_users.email, 'onboarding_approved', {

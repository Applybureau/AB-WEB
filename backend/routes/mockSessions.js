...
router.post('/', authenticateToken, requireClient, async (req, res) => {
  try {
    ...
    try {
      ...
      if (client) {
        await sendEmail(client.email, 'mock_session_scheduled', {
    ...
    try {
      await sendEmail(process.env.ADMIN_EMAIL || 'admin@applybureau.com', 'new_mock_session_request', {
  ...
  try {
    ...
    try {
      ...
      if (status === 'scheduled' && meeting_link && clientEmail) {
        await sendEmail(clientEmail, 'mock_session_confirmed', {
        ...
        await sendEmail(clientEmail, 'mock_session_cancelled', {
          client_name: clientName,
          session_type: mockSession.session_type,
          scheduled_date: new Date(mockSession.scheduled_date).toLocaleDateString()
        });
...
router.post('/:id/feedback', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      if (clientEmail) {
        await sendEmail(clientEmail, 'mock_session_feedback', {
          client_name: clientName,
          session_type: mockSession.session_type,

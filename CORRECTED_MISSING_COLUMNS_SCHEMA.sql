-- CORRECTED MISSING COLUMNS SCHEMA
-- Based on actual table structure found in database
-- This script adds only the missing columns that are actually needed

-- =====================================================
-- 1. ADD MISSING COLUMNS TO consultation_requests
-- =====================================================

-- The table already has: name, email, phone, message, status, admin_notes
-- We need to add: admin_status, preferred_slots, confirmed_time
...
-- Add preferred_slots for time slot selection (JSONB format)
ALTER TABLE consultation_requests 
...
-- Add reschedule and waitlist tracking
ALTER TABLE consultation_requests 
...
BEGIN
    ...
    CHECK (status IN ('pending', 'confirmed', 'completed', 'cancelled', 'approved', 'rejected'));
EXCEPTION
    WHEN OTHERS THEN NULL;
END $$;
...
-- Insert sample consultation request for testing (using correct column name 'name')
...
AND column_name IN ('admin_status', 'preferred_slots', 'confirmed_time', 'reschedule_reason', 'waitlist_reason')
...
-- Show sample data (using correct column name 'name')
SELECT 
    id,
    name,
    email,
...
DO $$
BEGIN
    RAISE NOTICE '✅ CORRECTED MISSING COLUMNS SCHEMA APPLIED SUCCESSFULLY';
    RAISE NOTICE '📋 Added columns: admin_status, preferred_slots, confirmed_time to consultation_requests';
    ...
    RAISE NOTICE '⚠️  Note: consultation_requests uses "name" column, not "full_name"';
END $$;